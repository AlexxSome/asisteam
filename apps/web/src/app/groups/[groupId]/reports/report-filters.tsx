import { REPORT_PERIOD_LABELS, activityTypeLabel, type ReportFilter } from "@asisteam/core";

export function ReportFilters({ groupId, filter, types }: {
  groupId: string; filter: ReportFilter;
  ...
  return <form action={`/groups/${groupId}/reports`} method="get" className="space-y-4 rounded-lg border p-4">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <label>Ordenar por<select name="sort" defaultValue={filter.sort} className={input}>
    ...
    <label className="flex min-h-11 items-center gap-2"><input type="checkbox" name="include_inactive" value="true" defaultChecked={filter.include_inactive} />Incluir deportistas inactivos</label>
    <div className="flex flex-wrap items-center gap-4">
      <button type="submit" className="rounded-md bg-primary px-4 py-3 text-primary-foreground">Aplicar filtros</button>
      <a href={`/groups/${groupId}/reports`} className="underline">Restablecer filtros</a>
    </div>
  </form>;
}
