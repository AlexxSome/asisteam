...
it("vacío y justificados no presentan 0 %; ofrece acceso a temporada", async () => {
  ...
  mock.history.mockResolvedValue({ history, error: null });
  render(await MyHistoryPage({ params, searchParams: Promise.resolve({ period: "season" }) }));
  expect(screen.getByText("Aún no tienes actividades con asistencia registrada.")).toBeTruthy();
  expect(screen.getAllByText("Sin datos")).toHaveLength(2); expect(screen.queryByText("0.0 %")).toBeNull();
