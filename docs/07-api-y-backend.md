...
- `deactivate_membership(p_group_id,p_membership_id)` y `reactivate_membership(p_group_id,p_membership_id)` implementan aquí la gestión ADMIN de ACTIVE→INACTIVE e INACTIVE→ACTIVE. La salida voluntaria del titular sigue en su historia correspondiente. Bloquean grupo y personas para revalidar autorización, última ADMIN (409 `LAST_ADMIN`), límites de 500 memberships y 30 grupos, y consentimiento vigente del menor, incluidos los apoderados auto-incorporados. No se desactiva un GUARDIAN con pupilos ACTIVE/PENDING para preservar V2 (`guardian_has_active_wards`).
